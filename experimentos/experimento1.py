...
class CNN(nn.Module):
	def __init__(self, num_classes=4):
		...
		self.layer1 = nn.Sequential(
			nn.Conv2d(in_channels=3, out_channels=128, kernel_size=8, stride=1, padding=2),
			nn.ReLU(),
			nn.MaxPool2d(kernel_size=2, stride=2))
		self.layer2 = nn.Sequential(
			nn.Conv2d(in_channels=128,out_channels=64, kernel_size=6, stride=1, padding=2),
			nn.ReLU(),
			nn.MaxPool2d(kernel_size=2, stride=2))
		...
		self.layer4 = nn.Sequential(
			...
			nn.ReLU(),
			nn.MaxPool2d(kernel_size=2, stride=2))
		self.fc = nn.Linear(19*19*32, num_classes)
